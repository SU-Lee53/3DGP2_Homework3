...
void Texture::CreateShadowMap(ComPtr<ID3D12Device> pd3dDevice, ComPtr<ID3D12GraphicsCommandList> pd3dCommandList, UINT nWidth, UINT nHeight)
{
	m_nResourceTypes = RESOURCE_TYPE_TEXTURE2D;
	...
	::ZeroMemory(&d3dResourceDesc, sizeof(D3D12_RESOURCE_DESC));
	{
		...
		d3dResourceDesc.DepthOrArraySize = 1;
		d3dResourceDesc.MipLevels = 1;
		d3dResourceDesc.Format = DXGI_FORMAT_R32_TYPELESS;	// Depth 만 32비트
		d3dResourceDesc.SampleDesc.Count = 1;
		d3dResourceDesc.SampleDesc.Quality = 0;
		d3dResourceDesc.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
	...
	m_SRVCPUDescriptorHandle = TEXTURE->CreateSRV(shared_from_this());
	m_DSVCPUDescriptorHandle = TEXTURE->CreateDSV(shared_from_this());
}
...
D3D12_SHADER_RESOURCE_VIEW_DESC Texture::GetSRVDesc() const
{
	...
	case RESOURCE_TYPE_TEXTURE2D_ARRAY: //[]
		d3dShaderResourceViewDesc.Format = d3dResourceDesc.Format == DXGI_FORMAT_R32_TYPELESS ? DXGI_FORMAT_R32_FLOAT : d3dResourceDesc.Format;
